...
const COLUMNS: TableColumn[] = [
  ...
  { field: 'text', header: "Text", sortable: true, searchable: true, filterType: 'text' },
  { field: 'noteDate', header: "Note Date", sortable: true, filterType: 'datepicker', html: (row: Record<string, unknown>) => row.noteDate ? new Date(String(row.noteDate)).toLocaleDateString() : '—' },
  { field: 'customerId', header: "Musteri", sortable: true },
];
...
export const NoteList: React.FC = () => {
  const list = useListQuery<NoteRecord>({ resource: 'Note' });
  const [showCreate, setShowCreate] = useState(false);
  const [editRecord, setEditRecord] = useState<NoteRecord | null>(null);
  const [selectedRows, setSelectedRows] = useState<NoteRecord[]>([]);

  const del = useDeleteMutation('Note');
  ...
  return (
    <>
      <ListPageLayout
        ...
        onSelectionChange={(rows) => setSelectedRows(rows as NoteRecord[])}
        ...
        headerActions={<>
          ...
          <TkButton label="+ Create Note" variant="primary" onTkClick={() => setShowCreate(true)} />
        </>}
      />
      ...
      />
      <NoteCreate open={showCreate} onClose={() => setShowCreate(false)} onSuccess={list.invalidate} />
      <NoteEdit record={editRecord} onClose={() => setEditRecord(null)} onSuccess={list.invalidate} />
