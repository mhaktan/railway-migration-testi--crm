...
export const NoteCreate: React.FC<NoteCreateProps> = ({ open, onClose, onSuccess }) => {
  const [form, setForm] = useState<Partial<NoteRecord>>({});
  const setField = (name: string, value: unknown) => setForm((p) => ({ ...p, [name]: value }));

  const mutation = useMutation({
    mutationFn: (values: Partial<NoteRecord>) => dataProvider.create('Note', values),
    ...
    <div style={overlayStyle} onClick={onClose}>
      <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
        <div style={{ padding: '20px 28px 0', flexShrink: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 18, fontWeight: 700 }}>Create Note</h2>
